...
def initialize_ai():
    try:
        ...
        # En stabil model sıralaması (Başına models/ eklemek 404'ü önler)
        for m_name in ['models/gemini-1.5-flash', 'models/gemini-1.5-pro', 'models/gemini-pro']:
            try:
                test_model = genai.GenerativeModel(m_name)
                # Küçük bir doğrulama testi
                test_model.generate_content("test", generation_config={"max_output_tokens": 1})
                return test_model, m_name
            except Exception as e:
                continue
        return None, "Uygun model bulunamadı veya API erişimi kısıtlı."
    except Exception as e:
        return None, str(e)

model, active_model = initialize_ai()

# --- 2. GENİŞLETİLMİŞ VERİ TABANI ---
AI_DIRECTORY = {
    "Yazılım ve Kodlama": {
...
with st.sidebar:
    ...
    if model:
        st.success(f"Bağlantı Aktif: {active_model.split('/')[-1]}")
    else:
        st.error("Bağlantı Kesik")
    st.markdown("---")
    st.caption("Versiyon: 2.6.0")
...
if st.button("En Uygun AI'ı Bul"):
    if not model:
        st.error(f"Başlatma Hatası: {active_model}")
        st.stop()
    ...
    if query:
        with st.spinner('Analiz ediliyor...'):
            try:
                ...
                st.info("İpucu: Eğer hata 'Safety' ile ilgiliyse, isteğinizi daha genel kelimelerle yazın.")
    else:
